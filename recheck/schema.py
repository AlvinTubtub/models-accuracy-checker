...
This re-checks the invariants the pipeline claims to enforce (chronological dates,
no duplicates, consistent actual_close across models for the same date) from
scratch against the exported JSON.
"""
...
def parse_company_export(payload: dict, *, source_label: str = "") -> CompanyEvaluationExport:
    ...
    integrity concerns are collected as ValidationIssue warnings instead
    of raising.
    """
    ...
    # Chronological, strictly increasing, no duplicates
    ...
    # Cross-check actual_close consistency across models
