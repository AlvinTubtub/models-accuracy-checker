...
This module is written from scratch against the standard textbook
definitions. It does not import, copy, or depend on anything from the
original ForecastPH repository. Its only job is to let this repo compute
its own opinion of a model's accuracy from raw actual/predicted arrays, so
that opinion can be compared against whatever the original pipeline
reported, rather than trusted blindly.

Definitions used (deliberately spelled out, since this is the thing being
...
This module does not compute that denominator itself (see
`recheck/denominator.py` for an optional from-scratch recomputation from
raw OHLCV); here it is simply required as an input, exactly as MASE's
standard definition requires a pre-agreed naive-forecast scale.
"""
...
def compute_metrics(
    ...
    *,
    ...
    if not math.isfinite(mase_denominator) or mase_denominator <= 0.0:
        raise MetricComputationError("mase_denominator must be finite and strictly positive")
...
def mase_denominator_from_closes(development_closes: Sequence[float]) -> float:
    ...
    chronological Close values. Provided here, separately from
    `compute_metrics`, so callers can choose whether to trust a reported
    denominator or recompute it from raw OHLCV (see
    `recheck/denominator.py`).
    """
